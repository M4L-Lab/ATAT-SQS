...
void calc_all_corr(Array<Real> *pcorr, const Structure &str, const Array<Array<MultiCluster> > &eqclus, const rMatrix3d unitcell, const Array<Array<Array<Rea
l> > > &corrfunc) {
...
Real calc_objective_func(const Array<Real> &corr, const Array<Real> &tcorr, Real mysqstol, Real weightdist, Real weightnbpt, Real weightdecay, const Array<Re
al> &diam, const Array<int> &nbpt) {
  Array<Real> dcorr(tcorr.get_size());
...
int main(int argc, char *argv[]) {
  ...
  int dohelp=0;
  ofstream outFile("step_record.txt"); // Added by Yichen
  AskStruct options[]={
    ...
    {"-tcf","Input file defining the target multibody correlations (default: internally calculated values for fully disordered state)",STRINGVAL,&tcorrfilena
me},
...
*/
  ...
  for (int c=0; c<supercell.get_size(); c++) {
    ...
    for (int at=0; at<mc(c).str.atom_pos.get_size(); ) {
      ...
      for (int t=0; t<mc(c).nbcomp(at); t++) {
        ...
        int rnumfake=(int)round(rnum); // Yichen made this change for pass error with interge issue
        //cerr << rnum << " " << inum << endl;
        if (!near_zero((Real)inum-rnumfake)) ERRORQUIT("Impossible to match point correlations due to incompatible supercell size.");
  ...
  int tic=0;
  int real_tic=0; // Added by Yichen
  Real obj=best.obj;
  best.obj=MAXFLOAT;
  int count_cniu=1;
  ...
  while (1) {
    if (obj<best.obj) {
      ...
      best=mc(cc);
      ofstream strfile;
      stringstream out_cniu;
      out_cniu << count_cniu++;
      std::string str1_cniu = "bestsqsq-";
      str1_cniu += out_cniu.str();
      std::string str2_cniu = "bestcorr-";
      str2_cniu += out_cniu.str();
      char *cstr1_cniu = new char[str1_cniu.length()+1];
      char *cstr2_cniu = new char[str2_cniu.length()+1];
      strcpy(cstr1_cniu,str1_cniu.c_str());
      strcpy(cstr2_cniu,str2_cniu.c_str());
      open_numbered_file(strfile,cstr1_cniu,ip,".out");
      delete [] cstr1_cniu;
      strfile.setf(ios::fixed);
      strfile.precision(sigdig);
      write_structure(best.str,ulat,site_type_list,atom_label,axes,strfile);
      ofstream corrfile;
      open_numbered_file(corrfile,cstr2_cniu,ip,".out");
      delete [] cstr2_cniu;
      ...
      count_cqian++; // Added by Yichen
      /// real_tic = maxtic-tic; // Added by Yichen
      if (!outFile.is_open()) { // Added by Yichen
        cerr << "Failed to open file." << endl; // Added by Yichen
        return 1; // Added by Yichen
       } // Added by Yichen
      outFile << count_cqian << " " << real_tic << endl;// Added by Yichen
    }
    ...
    //  cerr << endl << flush;
    tic--;
    real_tic++; // Added by Yichen
  }
  outFile.close(); // Added by Yichen
}
